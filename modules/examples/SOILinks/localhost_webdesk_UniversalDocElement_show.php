...
$NOTEST = true;
global $LF;
$requests_string = '{"0":"332","id":"332","1":"localhost_webdesk","nname":"localhost_webdesk","2":"_NODATA","ndatalabel":"_NODATA","3":"","nscope":"","4":"","ttable":"","5":"","tfields":"","6":"","tselect":"","7":"","tcode":"","8":"public","tlabel":"public","9":1,"bread":1,"10":0,"bdelete":0,"oid":"LINKS_request-localhost_webdesk--3-332--UD|3|AL|1"}';
...
$request = JSON_decode( $requests_string, true);
 if ( !$request) { echo 'help!'.substr( $requests_string, 0, 1000); die();}
$data_all[ L_getLabel( $request)] = get_requested_data( $links, $oid_str, $request);
$requests_string = '{"0":"346","id":"346","1":"UniversalDocElement_show","nname":"UniversalDocElement_show","2":"@3body@3main@3content@3middleColumn@3scroll@3document","ndatalabel":"@3body@3main@3content@3middleColumn@3scroll@3document","3":"","nscope":"","4":"UniversalDocElement","ttable":"UniversalDocElement","5":"* tlabel","tfields":"* tlabel","6":"","tselect":"","7":"","tcode":"","8":"owns","tlabel":"owns","9":1,"bread":1,"10":1,"bdelete":1,"oid":"LINKS_request-localhost_webdesk-LINKS_request-universaldocelement_show--3-332-3-346--AL|5"}';
$request = JSON_decode( $requests_string, true);
 if ( !$request) { echo 'help!'.substr( $requests_string, 0, 1000); die();}
$data_all[ L_getLabel( $request)] = get_requested_data( $links, $oid_str, $request);
...
// localhost_webdesk on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
//  includes on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["_NODATA"];
$OUTPUT=array();
$LF->currentBlock = '';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// all_all on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["_NODATA"];
$OUTPUT=array();
$LF->currentBlock = '';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// session on data from SetOfValues;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// [{1}]header on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// content on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// Left tools on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// Document on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// Right tools on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// footer on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// Home on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["_NODATA"];
$OUTPUT=array();
$LF->currentBlock = '';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// Local functions on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["_NODATA"];
$OUTPUT=array();
$LF->currentBlock = '';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// leftTools on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["body/main/content/leftColumn"];
$OUTPUT=array();
$LF->currentBlock = 'body/main/content/leftColumn';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// meta on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["head"];
$OUTPUT=array();
$LF->currentBlock = 'head';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// terms on data from UniversalDocElement--21-109-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// UniversalDocElement_show on data from UniversalDocElement;
...
if ( LF_env( 'is_Anonymous') && LF_count( $data) < 3)
{ 
  $LF->out( LINKSAPI::startTerm."You are no longer connected".LINKSAPI::endTerm);
  $LF->out( LINKSAPI::startTerm."Your will be redirected to the home page shortly".LINKSAPI::endTerm);  
  $LF->onload( "setTimeout( function() { window.close();}, 2000);");
}
else
{
  // Banner message
  $LF->onload( "new UDapiRequest( { ref:'show', command:'pageBanner( #set#,#=UD_docFull...innerHTML#);', quotes:'##'});");
   
  // Get id of document 
  $oid = LF_stringToOid( LF_env( 'oid'));
  $id = $oid[ LF_count( $oid) - 2];

  // Initiate ENVironmental variables for this document
  LF_env( "UD_{$id}_firstTime", LF_date());  
  ...
  {
     $element = $data[$i];
     if( $id == val( $element, 'id')) $include = true;
     if ( $include) $elementList[] = $data[$i]['nname'];
  }
  LF_env( "UD_{$id}_elementList", $elementList);
  
  // Look at permissions
  $params = LF_stringToOidParams( $data[1]['oid']);
  $permissions = (int) $params[0]['AL'];
  $mode = "display";
  if ( $permissions & 2) $mode = "edit";
  // Look at requested part
  $displayPart = "default";
  if ( LF_env( 'display'.$id)) $displayPart = LF_env( 'display'.$id);
  LF_env( 'display'.$id, "");
  $context = [ "mode"=>$mode, "displayPart"=>$displayPart];
  //if ( $LF->isMobile) $context['enablePaging'] = 0;
  // Use UniversalDoc class to build output
  $ud = new UniversalDoc( $context);  
  $ud->loadData( LF_env('oid'), $data);
  $LF->out( $ud->initialiseClient());
  // Build input script for accepting changes on elements
  $_SESSION["INPUT_UDE_FETCH"] = "if ( \$INPUT_DATA[1]['tcontent']) \$INPUT_DATA[1]['tcontent']=rawurldecode( \$INPUT_DATA[1]['tcontent']);return true;";
