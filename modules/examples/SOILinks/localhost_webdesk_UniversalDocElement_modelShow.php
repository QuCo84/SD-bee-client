...
$request = JSON_decode( $requests_string, true);
 if ( !$request) { echo 'help!'.substr( $requests_string, 0, 1000); die();}
$data_all[ L_getLabel( $request)] = get_requested_data( $links, $oid_str, $request);
$requests_string = '{"0":"378","id":"378","1":"UniversalDocElement_modelShow","nname":"UniversalDocElement_modelShow","2":"@3body@3main@3content@3middleColumn@3scroll@3document","ndatalabel":"@3body@3main@3content@3middleColumn@3scroll@3document","3":"","nscope":"","4":"","ttable":"","5":"","tfields":"","6":"","tselect":"","7":"","tcode":"","8":"owns","tlabel":"owns","9":1,"bread":1,"10":1,"bdelete":1,"oid":"LINKS_request-localhost_webdesk-LINKS_request-universaldocelement_modelshow--3-332-3-378--AL|5"}';
$request = JSON_decode( $requests_string, true);
 if ( !$request) { echo 'help!'.substr( $requests_string, 0, 1000); die();}
$data_all[ L_getLabel( $request)] = get_requested_data( $links, $oid_str, $request);
...
// localhost_webdesk on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
//  includes on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["_NODATA"];
$OUTPUT=array();
$LF->currentBlock = '';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// all_all on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["_NODATA"];
$OUTPUT=array();
$LF->currentBlock = '';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// [{1}]header on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// content on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// Left tools on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// Document on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// Right tools on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// footer on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// Home on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["_NODATA"];
$OUTPUT=array();
$LF->currentBlock = '';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// Local functions on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["_NODATA"];
$OUTPUT=array();
$LF->currentBlock = '';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// leftTools on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["body/main/content/leftColumn"];
$OUTPUT=array();
$LF->currentBlock = 'body/main/content/leftColumn';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// meta on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
$data = $DATA = $data_all["head"];
$OUTPUT=array();
$LF->currentBlock = 'head';
ob_start();
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// terms on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// UniversalDocElement_modelShow on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
...
// if !is_Anonymous && auto_logout_delay < x && rememberMe redirect to home
//if (LF_env( 'action') == "AJAX_modelShow")
{
  ...
  {
     LF_env( 'currentDirName', LF_env( 'ReqCurrentFTPdir'));
     LF_env( 'ReqCurrentFTPdir', "");
     $useFTP = true; 
  }
  else
  {  
     // Get this directory's element
  ...
  // Could/should be in model
  $LF->onload( "new UDapiRequest( {ref:'AJAX_show', command:'pageBanner(/set/,/$banner/);', quotes:'//' });");
  $ud->initialiseClient();
}

// Display FTP navigation
$currentDir = LF_env( 'currentDirName');
$ftpSpaceName = LF_env( 'ftpSpaceName');
$linkToFTPhome = "/webdesk/".LF_env('oid')."/AJAX_listFiles/ReqCurrentFTPdir|/";
// if ( !$ftpSpaceName) $ftpSpaceName= LINKSAPI::startTerm."Private FTP space".LINKSAPI::endTerm;
$navigation = "<h1>{$ftpSpaceName}</h1>"; // Param
if ( $currentDir && $currentDir != "FTP") $navigation .= "<h2>{$currentDir}</h2><a href=\"\">{$linkToFTPhome}</a>";
...
foreach ($OUTPUT AS $key=>$content) $LF->out( $content, $key);
if (val( $OUTPUT, 'STOP')) return true;$OUTPUT=array();
// www@6rfolks@6com_all_all_all_footer on data from UniversalDocElement--21-2119-21--UD|3|NO|OIDLENGTH|CD|5;
